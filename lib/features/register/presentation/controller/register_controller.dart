...
import '../../data/model/stateModel.dart';
...
class RegisterController extends GetxController {
 ...
 /* void _onStateChanged() {
    ...
    if (selectedStateName.isEmpty) {
      // Clear district and city if state is cleared
      districtCtrl.clear();
      cityCtrl.clear();
      districtList.clear();
      cityList.clear();
    ...
    final selectedState = statesData.firstWhere(
          (state) => state.name == selectedStateName,
      orElse: () => StateModel(id: '', name: '', districts: []),
    );

    if (selectedState.name.isNotEmpty) {
      // Update district list
      districtList.value = selectedState.districts
          .map((district) => district.name)
          .toList();

      // Clear district and city controllers
      districtCtrl.clear();
      cityCtrl.clear();
      cityList.clear();
    } else {
...
/*  void _onDistrictChanged() {
    ...
    if (selectedStateName.isEmpty || selectedDistrictName.isEmpty) {
      cityCtrl.clear();
      cityList.clear();
      return;
    }
    ...
    if (selectedState.name.isNotEmpty) {
      ...
      final selectedDistrict = selectedState.districts.firstWhere(
            (district) => district.name == selectedDistrictName,
        orElse: () => DistrictModel(id: '', name: '', cities: []),
      );

      if (selectedDistrict.name.isNotEmpty) {
        // Update city list
        cityList.value = selectedDistrict.cities
            .map((city) => city.name)
            .toList();

        // Clear city controller
        cityCtrl.clear();
      } else {
        cityList.clear();
      }
...
/// REGISTER ACTION
  /*   void onRegister() {
    ...
    final registerData = {
      ...
      "address": {
        "house": addressCtrl.text.trim(),
        "street": nearbyLocationCtrl.text.trim(),
    ...
    /// 🔹 For now logging (replace with API call)
  ...
  Future<void> onRegister() async {
    ...
    try {
      ...
      final requestBody = {
        ...
        "occupation": occupationCtrl.text.trim(),
        "reffral_code": referralCtrl.text.trim().isEmpty ? null : referralCtrl.text.trim(),
        "address": addressCtrl.text.trim(),
        "nearby_location": nearbyLocationCtrl.text.trim(),
        "pincode": pinCodeCtrl.text.trim(),
        "road_number": roadNumberCtrl.text.trim(),
        "state": stateCtrl.text.trim(),
        "city": cityCtrl.text.trim(),
        ...
        "user_type": userTypeCtrl.text.trim(),
        "cast_certificate": castCertificateData,
        "term_condition": true,
      };
      ...
      print("registration response : "+requestBody.toString());
